...
// Get revenue data for finance screen
  ...
  // Get all ingredients for manager view
  Future<List<Product>> getIngredients() async {
    ...
    try {
      ...
      print('⚠️ Error fetching ingredients: $e');
      return [];
    }
  }

  // Get outlet stock for a specific outlet
  Future<List<Map<String, dynamic>>> getOutletStock(String outletId) async {
    if (!_isInitialized) {
      throw Exception('SupabaseService not initialized');
    }
    ...
    try {
      ...
      print('⚠️ Error fetching outlet stock: $e');
      return [];
    }
  }

  // Allocate product from showcase to outlet
  Future<void> allocateProductToOutlet({
    required String productId,
    required String fromOutletId,
    required String toOutletId,
    required int quantity,
    ...
    try {
      await _client.from('product_allocations').insert({
        'product_id': productId,
        'from_outlet_id': fromOutletId,
        'to_outlet_id': toOutletId,
        'quantity': quantity,
      ...
      print('✅ Product allocation created successfully');
    } catch (e) {
      print('❌ Error allocating product: $e');
      throw Exception('Failed to allocate product: $e');
    }
  }

  // Get pending returns for outlet
  Future<List<Map<String, dynamic>>> getPendingReturns(String outletId) async {
    if (!_isInitialized) {
      throw Exception('SupabaseService not initialized');
    }
    ...
    try {
      final response = await _client
          .from('product_returns')
          .select('id, product_id, products(name), quantity, reason, status, created_at')
          .eq('to_outlet_id', outletId)
          .eq('status', 'pending')
          .order('created_at', ascending: false);
      ...
      print('⚠️ Error fetching pending returns: $e');
      return [];
    }
  }

  // Process product return
  Future<void> processProductReturn({
